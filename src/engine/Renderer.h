...
class Renderer
{
    public:
        ...
        void DrawTerrain(Terrain* terrain, const glm::vec2 & position = glm::vec2(0.0f), float metersPerPixel = 1.0f, bool manageLod = false, float maxDistance = 30.0f, int maxLod = 64, bool getTriangles = false, ShaderProgram* ovrShader = NULL);
		void DrawProject(Project* proj, bool manageLod = false, float maxDistance = 30.0f, int maxLod = 64, bool getTriangles = false, ShaderProgram* ovrShader = NULL);
		...
		unsigned int GetLastFrameTriangles() const;
// BEGIN_MARIANO
...
// END_MARIANO
    private:
		...
		ShaderProgram* _terrainGeomShader;
		ShaderProgram* _offsetParallaxShader;
		ShaderProgram* _offsetParallaxGeomShader;
		...
		GLuint _atomic;
		unsigned int _lastTriangles;
// BEGIN_MARIANO
		GLuint _fps; // Referencia al objeto de consulta de la GPU (se utilizará para medir el tiempo de GPU)
// END_MARIANO
};
