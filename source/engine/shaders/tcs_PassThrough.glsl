...
void main()
{
	...
	tcBoundingBox[gl_InvocationID] = vBoundingBox[gl_InvocationID];
	gl_out[gl_InvocationID].gl_Position = gl_in[gl_InvocationID].gl_Position;

	if (uUseAtomic)
	{
		for (int i = 0; i < 2*pow(gl_TessLevelInner[0]+1,2); i++)
		{
